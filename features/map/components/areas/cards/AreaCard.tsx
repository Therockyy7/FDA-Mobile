...
function severityColor(status: string) {
  return AREA_STATUS_COLORS[status as keyof typeof AREA_STATUS_COLORS] || "#9CA3AF";
}
function severityLabel(status: string) {
  return AREA_STATUS_LABELS[status as keyof typeof AREA_STATUS_LABELS] || "Không rõ";
}
function severityIcon(status: string) {
  return AREA_STATUS_ICONS[status as keyof typeof AREA_STATUS_ICONS] || "help-circle";
}
...
function StationChip({ code, level, color }: { code: string; level: number; color: string }) {
  return (
...
export function AreaCard({ area, onClose, onViewDetails, onEdit, onDelete }: AreaCardProps) {
  ...
  const maxLevel = area.contributingStations?.reduce((m, s) => (s.waterLevel > m ? s.waterLevel : m), 0) ?? 0;
  const wc = waterColor(maxLevel);
  const fill = Math.min((maxLevel / 60) * 100, 100);
    ...
    <View style={[styles.root, { backgroundColor: c.bg }]}>
      ...
      <View style={[styles.header, { backgroundColor: color }]}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.headerName} numberOfLines={1}>{area.name}</Text>
          {area.addressText && (
            <View style={styles.headerAddress}>
              <Ionicons name="location-outline" size={10} color="rgba(255,255,255,0.7)" />
              <Text style={styles.headerAddressText} numberOfLines={1}>{area.addressText}</Text>
            </View>
          )}
        </View>
      ...
      <View style={styles.body}>
        {/* Water level row */}
        {maxLevel > 0 && (
          <View style={[styles.waterRow, { backgroundColor: c.surface, borderColor: `${wc}40` }]}>
            ...
            <View style={styles.waterRight}>
              <Text style={[styles.progressPct, { color: wc }]}>{Math.round(fill)}%</Text>
              <View style={styles.progressRow}>
                <View style={[styles.progressTrack, { backgroundColor: c.border }]}>
                  <View style={[styles.progressFill, { width: `${fill}%`, backgroundColor: wc }]} />
                </View>
              </View>
            </View>
          </View>
        ...
        <View style={styles.metaRow}>
          <View style={[styles.metaItem, { backgroundColor: c.surface }]}>
            <MaterialCommunityIcons name="radius-outline" size={13} color={c.muted} />
            <Text style={[styles.metaVal, { color: c.text }]}>
              {area.radiusMeters >= 1000 ? `${(area.radiusMeters / 1000).toFixed(1)}km` : `${area.radiusMeters}m`}
            </Text>
            <Text style={[styles.metaLabel, { color: c.muted }]}>Bán kính</Text>
          </View>
          <View style={[styles.metaItem, { backgroundColor: c.surface }]}>
            <Ionicons name="pulse" size={13} color={c.muted} />
            <Text style={[styles.metaVal, { color: c.text }]}>{area.contributingStations.length}</Text>
            <Text style={[styles.metaLabel, { color: c.muted }]}>Trạm</Text>
          </View>
          <View style={[styles.metaItem, { backgroundColor: c.surface }]}>
            <Ionicons name="time-outline" size={13} color={c.muted} />
            <Text style={[styles.metaVal, { color: c.text }]} numberOfLines={1}>{formatTime(area.evaluatedAt)}</Text>
        ...
        {area.contributingStations.length > 0 && (
          <View style={styles.chipsSection}>
            <Text style={[styles.chipsLabel, { color: c.muted }]}>Trạm ảnh hưởng</Text>
            <View style={styles.chipsRow}>
              {area.contributingStations.slice(0, 4).map((s, i) => (
                <StationChip key={i} code={s.stationCode} level={s.waterLevel} color={waterColor(s.waterLevel)} />
              ))}
              {area.contributingStations.length > 4 && (
                <View style={[styles.chip, styles.chipMore]}>
                  <Ionicons name="add" size={10} color={c.muted} />
                  <Text style={[styles.chipCode, { color: c.muted }]}>+{area.contributingStations.length - 4}</Text>
                </View>
              )}
            </View>
          </View>
        ...
        <View style={styles.actions}>
          {onEdit && (
            <TouchableOpacity onPress={onEdit} style={[styles.actionBtn, styles.actionOutline, styles.actionSmall]} activeOpacity={0.75}>
...
const styles = StyleSheet.create({
  ...
  headerLeft: { flex: 1 },
  headerMeta: { flexDirection: "row", alignItems: "center", gap: 4, marginBottom: 3 },
  headerStatus: { fontSize: 11, fontWeight: "700", color: "rgba(255,255,255,0.85)", letterSpacing: 0.5 },
  ...
  closeBtn: { width: 28, height: 28, borderRadius: 14, backgroundColor: "rgba(255,255,255,0.2)", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  metaItem: { flex: 1, flexDirection: "column", alignItems: "center", paddingVertical: 8, borderRadius: RADIUS.chip, gap: 2 },
  ...
  chipMore: { backgroundColor: "transparent", borderWidth: 1, borderColor: "#E2E8F0" },

  // Actions
  actions: { flexDirection: "row", gap: 6, marginTop: 4 },
  actionBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 9, borderRadius: RADIUS.button, gap: 5 },
  actionOutline: { borderWidth: 1.5, borderColor: "#E2E8F0", backgroundColor: "transparent" },
