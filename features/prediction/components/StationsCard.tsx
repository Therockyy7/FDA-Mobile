...
export function StationsCard({ stations }: Props) {
  ...
  return (
    <View style={{
      ...
      <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 14, paddingHorizontal: 16, gap: 8 }}>
        <View style={{
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 13, fontWeight: "800", color: text }}>Trạm đo lường</Text>
          <Text style={{ fontSize: 11, color: muted }}>{stations.length} trạm đang giám sát</Text>
        </View>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 10 }}>
        {stations.map((st, idx) => {
          ...
          return (
            <View key={st.stationId} style={{
              width: 160,
              backgroundColor: sub,
              borderRadius: 16, padding: 12,
              borderTopWidth: 3, borderTopColor: sev.color,
            }}>
              {/* Header */}
              <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10, gap: 6 }}>
                ...
                <Text style={{ fontSize: 12, fontWeight: "800", color: text, flex: 1 }}>{st.stationCode}</Text>
              </View>

              {/* Water level */}
              <View style={{ marginBottom: 8 }}>
                <Text style={{ fontSize: 11, color: muted, marginBottom: 2 }}>Mực nước</Text>
                <Text style={{ fontSize: 20, fontWeight: "900", color: sev.color }}>{st.waterLevel}m</Text>
              </View>

              <View style={{ height: 1, backgroundColor: divider, marginBottom: 8 }} />

              {/* Street */}
              <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 4 }}>
                <Ionicons name="location-outline" size={10} color={muted} style={{ marginTop: 1 }} />
                <Text style={{ fontSize: 10, color: muted, flex: 1, lineHeight: 14 }} numberOfLines={2}>
                  {st.street.name}
                </Text>
              </View>
